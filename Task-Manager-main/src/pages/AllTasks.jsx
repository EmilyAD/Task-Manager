...
import { Search, Sprout, Trophy, Plus } from "lucide-react";
          ...
          {Object.entries(plantsByCategory).map(([category,plants])=>(
            <div key={category} className="bg-white rounded-xl p-6 shadow-sm border">
              ...
              <div className="flex gap-2 mb-3 text-3xl">
                {plants.map(p=><span key={p.id}>{p.plantType}</span>)}
              </div>
              <p className="text-sm text-gray-500">{plants.length} plant grown</p>
            </div>
          ))}
        </div>
